...
sed '/^chrm/d' SJ.out.tab.Pass1.sjdb > SJ.out.tab.Pass1.sjdb
...
singularity exec --bind /data/jonesse3,$TMPDIR $STAR STAR \
      ...
      --sjdbFileChrStartEnd SJ.out.tab.Pass1.sjdb \
...
singularity exec --bind /data/jonesse3,$TMPDIR $STAR STAR \
      ...
      --genomeDir $TMPDIR/GenomeForPass2 \
      --readFilesIn $READS \
      --outFileNamePrefix 2.4.0g1_100_ \
      --genomeLoad NoSharedMemory \
      --outSAMtype BAM SortedByCoordinate

#copy bam and log files to my directory
cp *bam /data/jonesse3
mkdir -p /data/jonesse3/logs/2.4.0g1_100/2ndpass
cp *Log.out *Log.final.out /data/jonesse3/logs/2.4.0g1_100/2ndpass
